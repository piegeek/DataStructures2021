...
void FibonacciHeap<T>::consolidate() {
    ...
    const size_t degree_arr_size = std::floor(std::log(size()) / std::log(phi)) + 1;
    size_t deg;
    ...
    // std::shared_ptr<FibonacciNode<T>> it = min_node;
    // std::cout << "START" << std::endl;
    // do {
    //     if (it->key < min_node->key) min_node = it;
    //     it = it->right;
    //     std::cout << it->degree << std::endl;
